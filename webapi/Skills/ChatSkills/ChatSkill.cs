...
public class ChatSkill
{
    ...
    private readonly SqlSchemaProviderHarness _sqlSchemaProviderHarness;
    ...
    /// <param name="prompt">Prompt used to generate the response</param>
    /// <returns>The created chat message</returns>
    private async Task<ChatMessage> StreamResponseToClient(string chatId, string userId, BotResponsePrompt prompt, string input)
    {
        // Create the stream
        ...
        if (!string.IsNullOrWhiteSpace(dataSourceResult))
        {
            string answer = string.Join(" ", dataSourceResult.Split(Array.Empty<char>(), StringSplitOptions.RemoveEmptyEntries).ToList());
            var words = answer.Split(" ").ToList();
            await foreach (string contentPiece in words.ToAsyncEnumerable())
            {
                chatMessage.Content = chatMessage.Content + contentPiece + " ";
                await this.UpdateMessageOnClient(chatMessage);
            }
        }
        else
